                ...
                <div class="row">
                    ...
                    <div class="col-xs-12">
                        <div class="form-group">
                            ...
                            <select id="mk1" multiple class="form-control" style="width:100%"></select>
                        </div>
                    </div>
                    <div class="col-xs-12">
                        <div class="form-group">
                            <label>Group Kelas</label>
                            <input id="group1" class="form-control" style="max-width: 150px;"/>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    $('#btn_add').click(function () {
        ...
        $('#next_data').append('<tr id="tr'+no_element+'">' +
            '<td><div style="text-align:center;"><div id="barcode'+no_element+'"></div></div></td>' +
            '<td><input type="hidden" class="hide" id="input_barcode'+no_element+'"/>' +
            '<div class="row">' +
            '                    <div class="col-xs-12">' +
            '                        <div class="form-group">' +
            '                            <label>Lecturer</label>' +
            '                            <select id="lecturer'+no_element+'" class="input-lecturer" name="states[]" multiple="multiple" style="width:100%">' +
            '                            </select>' +
            '                        </div>' +
            '                    </div>' +
            '                    <div class="col-xs-12">' +
            '                        <div class="form-group">' +
            '                            <label>Mata Kuliah</label>' +
            '                            <select id="mk'+no_element+'" multiple class="form-control" style="width:100%"></select>' +
            '                        </div>' +
            '                    </div>' +
            '                    <div class="col-xs-12">' +
            '                        <div class="form-group">' +
            '                            <label>Group Kelas</label>' +
            '                            <input id="group'+no_element+'" class="form-control" style="max-width: 150px;"/>' +
            '                        </div>' +
            '                    </div>' +
            '                </div>' +
            '</td>' +
            '<td style="text-align:center;"><button class="btn btn-danger btn-delete" data-id="'+no_element+'"><i class="fa fa-trash-o" aria-hidden="true"></i></button></td></tr>');
    ...
    function lod_data_mk(id) {
        var select_id = $('#'+id);
        var url_get = "<?php echo base_url('get-daftarAllMK'); ?>";

        $.get(url_get, function (data) {
            var data_json = JSON.parse(data);

            for(var i=0;i<data_json.length;i++){
                select_id.append('<option value="'+data_json[i].MKKode+' - '+data_json[i].Nama+'">'+data_json[i].MKKode+' | '+data_json[i].Nama+'</option>');
            }
        });
    ...
    function save() {
        ...
        for(var i=0;i<arr_element.length;i++){
            ...
            var matakuliah = $('#mk'+arr_element[i]).val();
            var group_kelas = $('#group'+arr_element[i]).val();

            if(lecturer!=null){
                var data = {
                    'barcode' : $('#input_barcode'+arr_element[i]).val(),
                    'lecturer' : JSON.stringify(lecturer),
                    'matakuliah' : JSON.stringify(matakuliah),
